...
serve: module-check ## Boot the development server.
	hugo serve --minify --buildFuture --environment development --disableFastRender --renderToMemory
...
container-serve: module-check ## Boot the development server using container.
	$(CONTAINER_RUN) --cap-drop=ALL --cap-add=AUDIT_WRITE --read-only \
		...
		hugo serve --minify --renderToMemory --buildFuture --environment development --bind 0.0.0.0 --destination /tmp/public --cleanDestinationDir --noBuildLock
